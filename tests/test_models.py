...
class TestDoculect:
    ...
    def test_doculect_belongs_to_family(self, dbsession, setup_models_once_for_test_module, man_id, expected_result):
...
class TestFamily:
    ...
    def test_family_has_doculects_with_feature_profiles(self, dbsession, setup_models_once_for_test_module, man_id,
                                                        expected_result):
    ...
    def test_family_is_descendant_of(self, dbsession, setup_models_once_for_test_module, man_id_of_presumable_parent,
                                     expected_result):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ('north_sem', [
                'ancient_hebrew', 'modern_hebrew', 'phoenician', 'neo_aramaic_of_maalula', 'neo_mandaic', 'turoyo',
                'official_aramaic', 'jewish_palestinian_aramaic', 'classical_mandaic', 'classical_syriac', 'ugaritic'
            ]),  # ugaritic is a top-level doculect, others are doculects in subfamilies (included nested ones)
            ('inuit', [])
        ])
    def test_family_iter_doculects_that_have_feature_profiles(self, dbsession, setup_models_once_for_test_module,
                                                              man_id, expected_doculect_ids):
        family = dbsession.scalars(select(models.Family).where(models.Family.man_id == man_id)).one()
