...
from langworld_db_pyramid.views.doculects_list import (
    get_doculects_by_substring,
    view_all_doculects_list,
)
...
NUMBER_OF_TEST_DOCULECTS_WITH_FEATURE_PROFILES = (
    338  # only those (out of 429) that have has_feature_profile set to '1'
)
NUMBER_OF_TEST_TOP_LEVEL_FAMILIES_WITH_FEATURE_PROFILES = (
    11  # only 11 out of 13 have doculects with feature profiles
)


def test_get_doculect_from_params_returns_doculect(
...
def test_get_doculect_from_params_returns_none_without_item_in_params(
    dummy_request, setup_models_once_for_test_module
):
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "en",
            ("afrikaans", "dutch"),
        ),  # 'dut' is in aliases for Afrikaans and in main name for Dutch
...
def test_get_doculects_by_substring_returns_empty_list_if_nothing_found(
    dummy_request, setup_models_once_for_test_module
):
...
def test_view_all_doculects_map_with_show_doculect(
    dummy_request, setup_models_once_for_test_module
):
    dummy_request.params["show_doculect"] = "french"
...
def test_view_all_doculects_map_without_show_doculect(
    dummy_request, setup_models_once_for_test_module
):
...
def test_families__get_family_immediate_subfamilies_and_icons(
    ...
    setup_models_once_for_test_module,
    family_man_id,
    expected_number_of_subfamilies,
...
def test_mapbox_token_get_mapbox_token(
    app_request,
):  # dummy_request won't have access to request.registry.settings
...
def test_view_families_for_map(
    ...
    assert (
        sum(len(group["doculects"]) for group in immediate_subfamilies)
        == expected_number_of_doculects
    )
...
def test_features_view_all_features_list_by_category(
    dummy_request, setup_models_once_for_test_module
):
...
def test_features_view_feature_map_of_values(
    dummy_request, dummy_config, setup_models_once_for_test_module
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # family + feature:
        (
            {"family": "yupik,aram", "A-11-1": "A-11-1"},
            6,
            [
                "official_aramaic",
        ...
        (
            {"family": "yupik,aram", "A-11-1": "A-11-1,A-11-2"},
            6,
            [
                "official_aramaic",
                "jewish_palestinian_aramaic",
        ...
        (
            {"family": "yupik,aram", "A-11-1": "A-11-1,A-11-7"},
            7,
            [
                "official_aramaic",
                "jewish_palestinian_aramaic",
        ...
        # family + 2 features:
        (
            {"family": "yupik,aram", "A-11-1": "A-11-1,A-11-7", "N-5": "N-5-1"},
            5,
            [
                "official_aramaic",
                "jewish_palestinian_aramaic",
        ...
        (
            {"family": "yupik,aram", "A-11-1": "A-11-1,A-11-7", "N-5": "N-5-1,N-5-3"},
            7,
            [
                "official_aramaic",
                "jewish_palestinian_aramaic",
