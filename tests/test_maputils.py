...
from langworld_db_pyramid.maputils.markers import (
...
def test_generate_map_icons():
    output_icons = [
        icon
        for icon, _ in zip(icons.generate_map_icons(), range(NUMBER_OF_POSSIBLE_SHAPES_AND_COLORS))
    ]
...
def test__generate_marker_group_item(app_request, dbsession, setup_models_once_for_test_module):
    ...
    assert (
        data.longitude == float(doculect.longitude) + 360
    )  # testing doculect with longitude -173.128
    assert (
        data.popupText
        == f'<a href="/en/doculect/{doculect.man_id}">{doculect.name_en}</a><br/>foo'
    )
...
def test_generate_marker_group(dbsession, app_request, setup_models_once_for_test_module):
    ...
    assert (
        eskimo_data["longitude"] == float(eskimo.longitude) + 360
    )  # testing doculect with longitude -173.128
    assert (
        eskimo_data["popupText"]
        == f'<a href="/en/doculect/{eskimo.man_id}">{eskimo.name_en}</a><br/>foo'
    )
...
def test_icon_for_object_fails_for_more_than_max_number():
    with pytest.raises(ValueError, match="Cannot generate more than"):
        _ = icons.icon_for_object(
            [DummyObject(i, i) for i in range(NUMBER_OF_POSSIBLE_SHAPES_AND_COLORS + 1)]
        )
